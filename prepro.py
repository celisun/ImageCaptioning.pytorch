...
/images is (N,3,224,224) uint8 array of raw image data in RGB format   :: we change to 224, 224
...
import imageio  # older scipy.misc deprecated, use imageio, skimage instead
import skimage
...
def prepro_captions(imgs):
  ...
  for i,img in enumerate(imgs):
    ...
    for j,s in enumerate(img['captions']):
      ...
      if not txt: continue  # added
      img['processed_tokens'].append(txt)
      if i < 10 and j == 0: print (txt)

    for x in img['processed_tokens']:
      if not x:
        print("ERROR MESSAGE: EMPTY PROCESSED TOKENS")
...
def build_vocab(imgs, params):
  ...
  for img in imgs:
    ...
    for txt in img['processed_tokens']:
      ...
      if not caption:
        print ("DEBUG WRONG MESSAGE ======: {}".format(txt))
...
def main(params):
  ...
  for i,img in enumerate(imgs):
    # load the image
    I = imageio.imread(os.path.join(params['images_root'], img['file_path']))
    try:
        Ir = skimage.transform.resize(I, (224, 224))
    except:
    ...
    # and swap order of axes from (224,224,3) to (3,224,224)
...
if __name__ == "__main__":
  ...
  # input json
  parser.add_argument('--input_json', default=os.path.join("data", "test_raw.json"), help='input json file to process into hdf5')
  parser.add_argument('--num_val', default=0, type=int, help='number of images to assign to validation data (for CV etc)')
  parser.add_argument('--output_json', default='test.json', help='output json file')
  parser.add_argument('--output_h5', default='test.h5', help='output h5 file')
  
  # options
  parser.add_argument('--max_length', default=16, type=int, help='max length of a caption, in number of words. captions longer than this get clipped.')
  parser.add_argument('--images_root', default='data', help='root location in which images are stored, to be prepended to file_path in input json')
